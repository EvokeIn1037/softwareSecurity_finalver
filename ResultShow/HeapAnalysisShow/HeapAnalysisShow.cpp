// HeapAnalysisShow.cpp : 定义应用程序的入口点。
//

#include "framework.h"
#include "HeapAnalysisShow.h"
#include "string.h"
                     ...
                     _In_ int       nCmdShow)
{
    ...
    LoadStringW(hInstance, IDC_HEAPANALYSISSHOW, szWindowClass, MAX_LOADSTRING);
    ...
    HACCEL hAccelTable = LoadAccelerators(hInstance, MAKEINTRESOURCE(IDC_HEAPANALYSISSHOW));
...
ATOM MyRegisterClass(HINSTANCE hInstance)
{
    ...
    wcex.hIcon          = LoadIcon(hInstance, MAKEINTRESOURCE(IDI_HEAPANALYSISSHOW));
    wcex.hCursor        = LoadCursor(nullptr, IDC_ARROW);
    wcex.hbrBackground  = (HBRUSH)(COLOR_WINDOW+1);
    wcex.lpszMenuName   = MAKEINTRESOURCEW(IDC_HEAPANALYSISSHOW);
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_PAINT:
    {
        ...
        for (i = 0; i < count; i++)
        {
            if ((strcmp(result[i].data, "heap")) == 0)
            {
                if (strcmp(result[i].hookMessage, "HeapCreate Hooked") == 0)
                {
                    heapCreateTime++;
                    strcpy_s(heapInfo, 100, result[i].dataContent[3]);
                }

                if (strcmp(result[i].hookMessage, "HeapDestroy Hooked") == 0)
                {
                    if (strcmp(result[i].dataContent[0], heapInfo) == 0)
                    {
                        heapDestroyTime++;
                        if (heapDestroyTime > heapCreateTime) {
                            ...
                            TextOutA(hdc, 0, (ii - iVscrollPos) * cyChar, "warning: Too many heap destruction times!", 41);
                            ii++;

                            len = strlen(heapInfo);
                            TextOutA(hdc, 0, (ii - iVscrollPos) * cyChar, heapInfo, len);
                            ii++;
                ...
                if (strcmp(result[i].hookMessage, "HeapFree Hooked") == 0)
                {
                    if (strcmp(result[i].dataContent[0], heapInfo) == 0)
                    {
                        heapFreeTime++;
                        if (heapFreeTime > heapCreateTime) {
                            TextOutA(hdc, 0, (ii - iVscrollPos) * cyChar, " ", 1);
                            ii++;
                            ...
                            TextOutA(hdc, 0, (ii - iVscrollPos) * cyChar, "warning: Too many heap free times!", 34);
                            ii++;
