// RegAnalysisShow.cpp : 定义应用程序的入口点。
//

#include "framework.h"
#include "RegAnalysisShow.h"
#include "string.h"
                     ...
                     _In_ int       nCmdShow)
{
    ...
    LoadStringW(hInstance, IDC_REGANALYSISSHOW, szWindowClass, MAX_LOADSTRING);
    ...
    HACCEL hAccelTable = LoadAccelerators(hInstance, MAKEINTRESOURCE(IDC_REGANALYSISSHOW));
...
ATOM MyRegisterClass(HINSTANCE hInstance)
{
    ...
    wcex.hIcon          = LoadIcon(hInstance, MAKEINTRESOURCE(IDI_REGANALYSISSHOW));
    wcex.hCursor        = LoadCursor(nullptr, IDC_ARROW);
    wcex.hbrBackground  = (HBRUSH)(COLOR_WINDOW+1);
    wcex.lpszMenuName   = MAKEINTRESOURCEW(IDC_REGANALYSISSHOW);
    ...
    case WM_PAINT:
        {
            ...
            for (i = 0; i < count; i++)
            {
                if ((strcmp(result[i].data, "reg")) == 0)
                {
                    if ((strcmp(result[i].hookMessage, "RegCreateKey Hooked")) == 0)
                    {
                        ...
                        TextOutA(hdc, 0, (ii - iVscrollPos) * cyChar, "New registry key created.", 25);
                        ii++;
                        ...
                        len = strlen(result[i].dataContent[1]);
                        TextOutA(hdc, 0, (ii - iVscrollPos) * cyChar, result[i].dataContent[1], len);
                        ii++;
                    ...
                    if ((strcmp(result[i].hookMessage, "RegOpenKey Hooked")) == 0)
                    {
                        if ((strcmp(result[i].dataContent[1], "lpSubKey : SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Run")) == 0)
                        {
                            TextOutA(hdc, 0, (ii - iVscrollPos) * cyChar, " ", 1);
                            ii++;
                            ...
                            TextOutA(hdc, 0, (ii - iVscrollPos) * cyChar, "warning: The program is modifying the startup key in the registry!", 66);
                            ii++;
                    ...
                    if ((strcmp(result[i].hookMessage, "RegSetValue Hooked")) == 0)
                    {
                        TextOutA(hdc, 0, (ii - iVscrollPos)* cyChar, " ", 1);
                        ii++;
                        ...
                        TextOutA(hdc, 0, (ii - iVscrollPos)* cyChar, "warning: Registry content has been changed!", 43);
                        ii++;
